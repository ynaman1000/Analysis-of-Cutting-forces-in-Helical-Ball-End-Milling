...
def plotter(x_data, y_data, fig_nm, subplot_rows, subplot_cols, y_max):
    ...
    for i in range(subplot_rows):
        if subplot_cols>1:
            ...
            for k in range(numE):
                l[k], = ax[i].plot(x_data.reshape((numN)), y_data[k, :, cnt].reshape((numN)), ls='-', lw=0.8, color=clrs[k])
    ...
    fig.legend(tuple(l) , ("Feedrate=0.2", "Feedrate=0.24", "Feedrate=0.28"), loc = 'lower center', ncol=6, labelspacing=0. )
